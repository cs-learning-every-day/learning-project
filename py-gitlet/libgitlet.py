...
class GitRepository(object):
    ...
    def __init__(self, path, force=False) -> None:
        self.worktree = path
        self.gitdir = os.path.join(path, ".gitlet")
...
def repo_create(path) -> GitRepository:
    ...
    # First, we make sure the path either doesn't exist or is an e mpty dir.
...
def repo_find(path=".", required=True):
    ...
    if os.path.isdir(os.path.join(path, ".gitlet")):
...
def object_read(repo: GitRepository, sha):
    """Read object object_id from Git repostiory repo. Return a
...
# Key value parse for commit object
def kvlm_parse(raw, start=0, dct=None):
    if not dict:
        dct = collections.OrderedDict()

    # search fro the next space and the next newline.
    ...
    # case find return -1), we assume a blank line. A blank line
    # means the remainder of the data is message.
    if (spc < 0) or (nl < spc):
        assert nl == start
        dct[b""] = raw[start + 1 :]
        return dct
...
def tag_create(repo, name, reference, type):
    ...
    if type == "object":
        ...
        tag.kvlm[
            b""
        ] = b"A tag generated by gitlet, which won't let you customize the message!"
